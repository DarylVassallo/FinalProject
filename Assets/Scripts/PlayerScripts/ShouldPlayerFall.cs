...
public class ShouldPlayerFall : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (add != null)
        {
            //Disables The Player's Gravity If The Reset Mechanic Is Not Being Used And fallingAmount Is Less Or Equal To 5
            if (add.isResetActivated == false && fallingAmount <= 5)
            {
                rb.isKinematic = true;
                rb.useGravity = false;
                fallingAmount = 0;
        ...
        {
            //Disables The Player's Gravity If fallingAmount Is Less Or Equal To 5
            if (fallingAmount <= 5)
            {
                rb.isKinematic = true;
                rb.useGravity = false;
